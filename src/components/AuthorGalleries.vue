<template>
  <div>
    ...
    <div class="alert alert-danger" v-if="!galleries.length">
...
export default {
  ...
  methods: {
    loadMore() {
      this.page++;
      galleryService
        .getUserGalleries(this.id, this.page, this.term)
        .then(galleries => {
  ...
  computed: {
    username() {
      return (
        this.galleries[0].user.first_name +
        " " +
        this.galleries[0].user.last_name
      );
    }
  },
  ...
  beforeRouteEnter(to, from, next) {
    next(vm => {
      ...
      galleryService
        .getUserGalleries(vm.id, vm.page, vm.term)
        .then(galleries => {
